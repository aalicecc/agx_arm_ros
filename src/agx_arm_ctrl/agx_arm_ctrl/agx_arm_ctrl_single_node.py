...
class AgxArmRosNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            JointState, "/control/joint_states", self.joint_callback, 1
        )
        self.create_subscription(
            PoseStamped, "/control/end_pose", self.pose_callback, 1
        )
    ...
    def _enable_arm(self, enable: bool = True, timeout: float = 5.0) -> bool:
        ...
        all_joints_in_target_status = (
            all(joints_status) if enable else not all(joints_status)
        )
    ...
    def joint_callback(self, joint_state: JointState):
        ...
        if joint_states is None or joint_states.hz <= 0 or not self.enable_flag:
            self.get_logger().warn(
                "Agx_arm is not connected or not enabled, cannot control end effector pose"
            )
            return
        ...
        self.agx_arm.move_j(
            [joint_pos.get(f"joint{i}", 0) for i in range(1, self.arm_joint_count + 1)]
        )
    ...
    def enable_callback(self, request, response):
        try:
            ...
            if joint_states is None or joint_states.hz <= 0:
                ...
                self.get_logger().warn(
                    "Agx_arm is not connected, cannot set enable state"
                )
            elif request.data:
    ...
    def move_home_callback(self, request, response):
        try:
            ...
            if joint_states is None or joint_states.hz <= 0:
                self.get_logger().warn(
                    "Agx_arm is not connected, cannot move to home position"
                )
                ...
                self.get_logger().warn(
                    "Agx_arm is not enabled, cannot move to home position"
                )
        except Exception as e:
